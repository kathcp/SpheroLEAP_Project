...
function Sphero(url) {
	...
	/* Additional function for usage with a LEAP device*/
	
	var arrayDegrees = [0,90,180,270];
	
	this.sendCommands = function(direction){
		
		var speed = 50;
		// turnValue = 0-F; 1-R; 2-B; 3-L
		var turnValue;
		...
		if (turnValue !== undefined){
			...
			console.log("Turn " + arrayDegrees[turnValue]);			
			var dir = arrayDegrees[turnValue];
			var turnCommand = {"command": "turn", "direction": dir};
			window.connection.send(turnCommand);
			var goForward = {"command": "rollForward", "speed": speed};
			window.connection.send(goForward);
					
			//Goes through the arrayDegrees
			for (var i = 0; i <= 3; i++){
				// it will decrease turnValue*90 from the array position 
				for (var j = turnValue; j > 0; j--){
					if (arrayDegrees[i] !== 0){        
						arrayDegrees[i] = arrayDegrees[i] - 90;
